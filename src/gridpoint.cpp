...
namespace Btwxt {
    ...
    void GridPoint::set_target(std::vector<double> v) {
        if (v.size() != ndims) {
            showMessage(MsgLevel::MSG_ERR, stringify("Target and Gridded Data do not have the same dimensions."));
        }
        target = v;
        target_is_set = true;
        set_floor();
    ...
    std::vector<double> GridPoint::get_current_target() {
        if (!target_is_set) {
          showMessage(MsgLevel::MSG_WARN, stringify("The current target was requested, but no target has been set."));
          return target;
        }
        return target;
    }
    ...
    void GridPoint::set_dim_floor(std::size_t dim)
    {
        ...
        if (target[dim] < axis.extrapolation_limits.first) {
            ...
            is_inbounds[dim] = Bounds::INBOUNDS;
            std::vector<double>::const_iterator upper = std::upper_bound(axis.grid.begin(), axis.grid.end(), target[dim]);
    ...
    void GridPoint::calculate_weights() {
        for (std::size_t dim = 0; dim < ndims; dim += 1) {
            double edge[] = {grid_data->grid_axes[dim].grid[point_floor[dim]], grid_data->grid_axes[dim].grid[point_floor[dim] + 1]};
    ...
    std::vector<double> GridPoint::get_results() {
        ...
        if (results.size() == 0u) {
            showMessage(MsgLevel::MSG_WARN, stringify("There are no value tables in the gridded data. No results returned."));
            return results;
        }
        std::fill(results.begin(),results.end(), 0.0);
        if (!target_is_set) {
          showMessage(MsgLevel::MSG_WARN, stringify("Results were requested, but no target has been set."));
          return results;
        }
        double weight;
    ...
    double GridPoint::get_vertex_weight(const std::vector<short> &v,
        ...
        for (std::size_t dim = 0; dim < ndims; dim++) {
            if (methods[dim] == Method::CUBIC) {
                ...
                terms[dim][1] = cubic_slope_coeffs[dim][flavor]
    ...
    double GridPoint::sum_weighting_terms() {
        ...
        std::size_t N = 1;
        for (std::size_t dim=0; dim < nT; ++dim) { N *= 2; } // Two options per dimension (normal or slope terms)
